...
class uniformBands():
    ...
    def __init__(self, x, a, y, z, n, typ, p1_x, py_1x, py_0x, pz_1x, pz_0x, t, u, iF_Y=None, iF_Z=None):
        """
        ...
        self.pz_1x = pz_1x
        self.pz_0x = pz_0x
        self.typ = typ # type may not be useful!
        self.t = t
        self.u = u
        self.iF_Y = iF_Y
        self.iF_Z = iF_Z
    ...
    def find_max_lcb_f(self):
        opt = nlopt.opt(nlopt.LN_COBYLA, self.dim)
        opt.set_lower_bounds(-1*np.ones(self.dim))
        opt.set_upper_bounds(np.ones(self.dim))
        opt.set_max_objective(self.lcb_f)
        opt.set_xtol_rel(5e-3)
        x_init = np.random.uniform(low=-1, high=1, size=self.dim)
        try:
